package ru.inspirit.steering 
{
	...
	{		
		...
		public var edgeBehavior:uint = EDGE_BOUNCE;
		...
		public function Vehicle(_position:SteerVector3D = null, behaviorList:BehaviorList = null) 
		{
			identity();
			if (_position) {
				position = _position;
		...
		public function boundsCheck():void
		{
			...
			if( distance > boundsRadius + vehicleRadius ) 
			{
				switch( edgeBehavior )
				{
					case EDGE_BOUNCE :
					{
						var _p:SteerVector3D = new SteerVector3D( position.x, position.y, position.z);
						_p.decrementBy(boundsCentre);
						_p.normalize();
						_p.scaleBy(boundsRadius + vehicleRadius);
						
						velocity.scaleBy(-1);
						_p.incrementBy(velocity);
						_p.incrementBy(boundsCentre);
						
						position = position.adding(_p);
